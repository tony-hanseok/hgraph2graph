...
def find_fragments(mol):
    ...
    for fragment in new_smiles.split('.'):
        ...
        try:  # TODO: can't kekulize
            indices = set([atom.GetAtomMapNum() for atom in fragment_mol.GetAtoms()])
        except AttributeError:
            continue
        fragment_mol = get_clique_mol(mol, indices)
        fragment_mol = sanitize(fragment_mol, kekulize=False)
        fragment_smiles = Chem.MolToSmiles(fragment_mol)
        hopts.append((fragment_smiles, indices))
...
def get_assm_cands(mol, atoms, inter_label, atoms_in_cluster, inter_size):
    ...
    position, icls = zip(*inter_label)
    # position: atom id

    if inter_size == 1:  # cluster간에 single atom으로 연결되어 있을 경우.
        anchor_atom = position[0]
        cands = [anchor_atom] + [x for x in atoms_in_cluster if rank[x] != rank[anchor_atom]]

    elif icls[0] == icls[1]:  # symmetric case
        shift = atoms_in_cluster[inter_size - 1:] + atoms_in_cluster[:inter_size - 1]
        cands = zip(atoms_in_cluster, shift)
        cands = [position] + [(x, y) for x, y in cands if
                         (rank[min(x, y)], rank[max(x, y)]) != (rank[min(position)], rank[max(position)])]
    else:
        shift = atoms_in_cluster[inter_size - 1:] + atoms_in_cluster[:inter_size - 1]
        cands = zip(atoms_in_cluster + shift, shift + atoms_in_cluster)
        cands = [position] + [(x, y) for x, y in cands if (rank[x], rank[y]) != (rank[position[0]], rank[position[1]])]
