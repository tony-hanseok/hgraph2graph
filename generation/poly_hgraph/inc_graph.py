...
class IncTree(IncBase):
    ...
    def __init__(self, batch_size, node_fdim, edge_fdim, max_nodes=100, max_edges=200, max_nb=12, max_sub_nodes=30):
...
class IncGraph(IncBase):
    ...
    def add_mol(self, batch_idx, smiles, inter_label, nth_child):
        ...
        for atom in emol.GetAtoms():  # atoms must be inserted in order given by emol.GetAtoms() (for rings assembly)
            if atom.GetIdx() in atom_map:
                ...
                assert idx == self.add_node(
                    self.get_atom_feature(new_atom))  # mol and nx graph must have the same indexing
        ...
        for bond in emol.GetBonds():
            ...
            if existing_bond is None:
                ...
                self.add_edge(a1, a2, self.get_mess_feature(bond.GetBeginAtom(), bond_type,
                                                            nth_child if a2 in attached else 0))  # only child to father node (in intersection) have non-zero nth_child
                self.add_edge(a2, a1,
    ...
    def get_assm_cands(self, cluster, used, smiles):
        ...
        if inter_size == 1:
            ...
            anchors = [a for a in attach_points if
                       is_anchor(emol.GetAtomWithIdx(a), [0])]  # all attach points are labeled with 1
            attach_points = [a for a in attach_points if a not in anchors]
            attach_points = [anchors[0]] + attach_points + [
                anchors[1]]  # force the attach_points to be a chain like anchor ... anchor
            anchor_smiles = [get_anchor_smiles(emol, a, idxfunc) for a in anchors]
