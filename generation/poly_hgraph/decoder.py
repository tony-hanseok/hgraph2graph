...
from generation.poly_hgraph.encoder import IncHierMPNEncoder
from generation.poly_hgraph.inc_graph import IncTree, IncGraph
...
class HierMPNDecoder(nn.Module):
    ...
    def __init__(self, vocab, atom_vocab, rnn_type, embed_size, hidden_size, latent_size, depthT, depthG, dropout,
                 attention=False):
        ...
        self.hmpn = IncHierMPNEncoder(vocab, atom_vocab, rnn_type, embed_size, hidden_size, depthT, depthG, dropout)
        ...
        self.topo_loss = nn.BCEWithLogitsLoss(reduction='sum')  # graph prediction
        self.cls_loss = nn.CrossEntropyLoss(reduction='sum')  # motif prediction
        self.icls_loss = nn.CrossEntropyLoss(reduction='sum')  # motif+attachment prediction
        self.assm_loss = nn.CrossEntropyLoss(reduction='sum')  # attachment prediction
    ...
    def enum_attach(self, hgraph, cands, icls, nth_child):
        ...
        cand_vecs = torch.cat([cand_vecs, icls_vecs, order_vecs], dim=-1)  # TODO: bug. cand_vec dim이 2차원
    ...
    def decode(self, src_mol_vecs, greedy=True, max_decode_step=100, beam=5):
        ...
        graph_batch = IncGraph(self.atom_vocab, batch_size, node_fdim=self.hmpn.atom_size,
